...
import { createWBSocket, EVENTS, WB_GAME_CODE } from './socketClient';
import WildBountyGrid from './WildBountyGrid';
...
import './WildBountyGame.css';

const DEFAULT_PLAYER_ID =
  process.env.REACT_APP_WB_PLAYER_ID ||
  process.env.REACT_APP_DH_PLAYER_ID ||
  '6895e9e558a4fb592a3a2e70';
const DEFAULT_GAME_ID = process.env.REACT_APP_WB_GAME_ID || '68a575815db8e6a18a884638';

export default function WildBountyGame() {
  ...
  // (see src/socket/index.js:668 for wild_bounty_96 fast-path).
  ...
  const logEvent = useCallback((eventName, payload, direction = 'in') => {
    ...
    console.log(`[WildBounty ${arrow} ${eventName}]`, payload);
  ...
  useEffect(() => {
    if (!playerId) return;
    const socket = createWBSocket(playerId);
  ...
  const handleVariantPick = useCallback((variantId) => {
    ...
    const picked = WB_VARIANT_OPTIONS.find((v) => v.id === variantId);
        ...
        >
          ...
          <div className="ag-login-row">
            ...
            <input
              ...
              placeholder="Wild Bounty Mongo ObjectId"
            />
          </div>
          <button className="ag-connect-btn" onClick={handleConnect}>ENTER THE UPRISING</button>
          <div className="ag-hint">
            Socket: {process.env.REACT_APP_WB_SOCKET_URL || 'http://localhost:4033'}
          </div>
        </motion.div>
      )}
