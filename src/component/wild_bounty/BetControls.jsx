...
import { WB_BET_SIZES, WB_BET_LEVELS } from './symbols';
...
export default function BetControls({
  ...
  const cycleBetSize = (delta) => {
    const idx = WB_BET_SIZES.indexOf(betSize);
    const next = idx + delta;
    if (next >= 0 && next < WB_BET_SIZES.length) setBetSize(WB_BET_SIZES[next]);
  };
  const cycleBetLevel = (delta) => {
    const idx = WB_BET_LEVELS.indexOf(betLevel);
    const next = idx + delta;
    if (next >= 0 && next < WB_BET_LEVELS.length) setBetLevel(WB_BET_LEVELS[next]);
  };
        ...
        <div className="ag-bet-cluster">
          <button
            ...
            onClick={() => {
              ...
              if (betLevel > WB_BET_LEVELS[0]) cycleBetLevel(-1);
              else cycleBetSize(-1);
            }}
          >−</button>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (betLevel < WB_BET_LEVELS[WB_BET_LEVELS.length - 1]) cycleBetLevel(1);
              else cycleBetSize(1);
            }}
          >+</button>
        </div>
