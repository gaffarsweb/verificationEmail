...
import { getSymbol, WB_REEL_ROWS } from './symbols';
...
/**
 * Cell with optional golden frame overlay (Wild Bounty mechanic — golden = wild reveal on win)
 */
...
export default function WildBountyGrid({ board, cascadeData, spinning, freeSpin, currentMultiplier, goldenFramePositions }) {
    ...
    <div className={`ag-grid-container ${freeSpin ? 'ag-fs-mode' : ''}`}>
      ...
      <div className={`ag-grid ${freeSpin ? 'ag-fs-grid' : ''}`}>
        {columns.map((col, cIdx) => {
          const reelRows = WB_REEL_ROWS[cIdx];
      ...
      {/* Persistent multiplier badge (Wild Bounty's doubling cascade mult) */}
      <motion.div
