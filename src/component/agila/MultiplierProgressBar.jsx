...
import { AG_MULT_TIERS } from './symbols';
...
export default function MultiplierProgressBar({ currentMultiplier = 1 }) {
  ...
  let activeIdx = AG_MULT_TIERS.findIndex((t) => t >= currentMultiplier);
  if (activeIdx === -1) activeIdx = AG_MULT_TIERS.length - 1;
  if (currentMultiplier < AG_MULT_TIERS[0]) activeIdx = 0;

  // Show a window of 5 tiers around the active one
  const windowStart = Math.max(0, activeIdx - 2);
  const windowEnd = Math.min(AG_MULT_TIERS.length, windowStart + 5);
  const visibleTiers = AG_MULT_TIERS.slice(windowStart, windowEnd);
